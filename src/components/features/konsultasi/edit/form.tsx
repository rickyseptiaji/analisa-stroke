...
export default function KonsultasiEditForm({
  konsultasiId,
}: {
  konsultasiId: string;
}) {
  ...
  const [konsultasi, setKonsultasi] = useState<any>(null);
  ...
  const fetchKonsultasi = async () => {
    const response = await fetch(`/api/konsultasi/${konsultasiId}`);
    const data = await response.json();
    console.log(data);
    if (!response.ok) {
      toast.error(data.message || "Gagal mengambil data konsultasi");
      return;
    }
    setKonsultasi(data);
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        await Promise.all([fetchKonsultasi(), fetchPasien(), fetchGejala()]);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  useEffect(() => {
    ...
    form.reset({
      pasienId: konsultasi.pasienId?.toString() || "",
      jawaban: konsultasi.details?.map((d: any) => d.kd_gejala) || [],
    });
  }, [konsultasi, form]);

  const pasienId = useWatch({
    control: form.control,
    name: "pasienId",
  });
  const jawaban = useWatch({
    control: form.control,
    name: "jawaban",
    defaultValue: [],
  });
          ...
          <div className="space-y-2">
            <div className="border rounded-lg overflow-hidden">
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {dataGejala.map((item) => {
                    const checked = jawaban?.includes(item.kd_gejala);
                          ...
                          <Checkbox
                            ...
                            onCheckedChange={(isChecked) => {
                              if (isChecked) {
                                ...
                                form.setValue(
                                  "jawaban",
                                  jawaban?.filter((v) => v !== item.kd_gejala)
                                );
                              }
                            }}
                          />
